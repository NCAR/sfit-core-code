...
!  --- TO BE CALCULATED AND CHECK FOR ARRAY OVERFLOWS
      ...
      ENDIF
      IF (F_MTCKD) THEN
         ALLOCATE (MTCKD_ABSRB(NMONSM), STAT=NAERR)
         IF (NAERR /= 0) THEN
            WRITE (16, *) 'INITIALIZE: COULD NOT ALLOCATE MTCKD_ABSRB ARRAY ERROR NUMBER = ', NAERR
            WRITE ( 0, *) 'INITIALIZE: COULD NOT ALLOCATE MTCKD_ABSRB ARRAY ERROR NUMBER = ', NAERR
            CALL SHUTDOWN
            STOP 2
         ENDIF
      END IF
...
!  --- INPUT ATMOSPHERIC LINE DATA FROM TAPE14
      ...
      WRITE(*,133) F_MTCKD
      WRITE(16,134) F_MTCKD
      
133   FORMAT( '     MTCKD CONTINUUM CALCULATED                               :',L3)
134   FORMAT( ' MTCKD CONTINUUM CALCULATED                               :',L3)
...
!-------------------------------------------------------------------------------
      ...
      IF( ALLOCATED( MTCKD_ABSRB ) )DEALLOCATE (MTCKD_ABSRB)
