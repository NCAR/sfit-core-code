...
!  --- TO BE CALCULATED AND CHECK FOR ARRAY OVERFLOWS
      ...
      !print*, 'init 1 allocate ', nret+1, kmax+ncell, ncross
      ALLOCATE (CROSS(NRET+1,KMAX+NCELL,NCROSS), STAT=NAERR)
      IF (NAERR /= 0) THEN
         ...
         STOP 2
      ENDIF
      ALLOCATE (CROSS_FACMAS(NRET+1,KMAX+NCELL,NMONSM), STAT=NAERR)
      IF (NAERR /= 0) THEN
